...
function SuppliesSection({ supplies, expanded, onToggle }) {
  return (
    <div className="border rounded-lg">
      <button
        onClick={onToggle}
        className="w-full p-4 text-left flex justify-between items-center hover:bg-gray-50"
      >
        <span className="font-semibold">🧰 Critical Supplies</span>
        <span>{expanded ? '−' : '+'}</span>
      </button>
      {expanded && (
        <div className="p-4 pt-0 space-y-2">
...
function EvacuationSection({ protocols, expanded, onToggle }) {
  return (
        <div className="border rounded-lg">
        <button
            onClick={onToggle}
            className="w-full p-4 text-left flex justify-between items-center hover:bg-gray-50"
        >
            <span className="font-semibold">🚪 Evacuation Protocols</span>
            <span>{expanded ? '−' : '+'}</span>
        </button>
        {expanded && (
            <div className="p-4 pt-0 space-y-4">
            {Object.entries(protocols).map(([location, steps], idx) => (
                <div key={idx}>
